...
export function useHeaderBar() {
  ...
  // 检查GitHub链接是否显示
  const shouldShowGithubLink = computed(() => {
    return isFeatureEnabled('githubLink')
  })

  // 检查PyPI链接是否显示
  const shouldShowPypiLink = computed(() => {
    return isFeatureEnabled('pypiLink')
  })
  ...
  return {
    ...
    shouldShowGithubLink, // 是否显示GitHub链接
    shouldShowPypiLink, // 是否显示PyPI链接
