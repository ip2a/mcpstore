...
""".strip(),
    ...
    def test_python_facade_keeps_session_api_shape(self):
        ...
        self.assertEqual(context.find_session_by_key(session.session_key), session)
        self.assertEqual(context.session_by_key(session.session_key), session)
        with self.assertRaisesRegex(ValueError, "Session not found"):
            context.session_by_key("store:global:missing")
...
""",
    ...
    def test_python_facade_session_state_comes_from_rust_core(self):
        ...
        attached_session = second.for_store().session_by_key(first_session.session_key)

        self.assertIsNotNone(second_session)
        self.assertEqual(second_session.session_key, first_session.session_key)
        self.assertEqual(attached_session.session_id, "shared")
        self.assertEqual(attached_session.session_key, first_session.session_key)
        ...
        self.assertEqual(attached_session.get_state("cursor").page, 1)
    ...
    def test_hub_http_can_expose_rust_service_management_tools(self):
        from mcpstore.config import MemoryConfig
        from mcpstore.core.store.rust_backend import RustStoreBackend
        ...
        with patch("mcpstore._rust_cli.resolve_rust_cli_binary", return_value="/bin/mcpstore"):
            with patch("mcpstore._rust_cli.resolve_runtime_cwd", return_value="/tmp"):
                with patch("mcpstore.core.store.rust_backend.subprocess.run", return_value=completed) as run:
                    code = store.for_store().hub_http(
                        block=True,
                        expose_service_tools=True,
                    )

        self.assertEqual(code, 0)
        cmd = run.call_args.args[0]
        self.assertIn("--expose-service-tools", cmd)
    ...
    def test_python_context_compatibility_exports_are_rust_backed(self):
        ...
        self.assertIn("list_services_async", dir(type(agent_proxy)))
        self.assertIn("show_config_async", dir(type(agent_proxy)))
        ...
        self.assertEqual(operations.find_session_by_key(shared_session_key).session_id, "shared")
        self.assertEqual(operations.session_by_key(shared_session_key).session_id, "shared")
        with patch.object(RustSession, "for_langchain", lambda session: session.session_id):
            ...
            self.assertEqual(operations.for_langchain_with_session_key(shared_session_key), "shared")
            ...
            self.assertEqual(context.for_langchain_with_session_key(shared_session_key), "shared")
