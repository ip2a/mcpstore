...
use mcpstore_config::{AppConfig, CacheBackend, ConfigManager, ServerConfig};
use mcpstore_events::{Event, EventBus};
use mcpstore_registry::{ConnectionStatus, ServiceEntry, ServiceRegistry};
use mcpstore_transport::client::ConnectionPool;
use mcpstore_transport::ToolDescription;
...
struct StoreRuntimeConfig {
    max_connection_attempts: i32,
    retry_backoff_base_secs: i64,
    retry_backoff_max_secs: i64,
    reconnect_hard_timeout_secs: i64,
    half_open_lease_secs: i64,
...
impl MCPStore {
    ...
    async fn connect_service_internal(&self, name: &str, automatic_retry: bool) -> Result<()> {
        ...
        let tool_infos: Vec<mcpstore_registry::ToolInfo> = tools
            .into_iter()
            .map(|t| mcpstore_registry::ToolInfo {
                name: t.name,
    ...
    pub async fn list_all_tools(&self) -> Vec<mcpstore_registry::ToolInfo> {
        self.refresh_from_db_if_needed().await.ok();
    ...
    ) -> Result<mcpstore_transport::ToolCallResult> {
        self.refresh_from_db_if_needed().await?;
        if self.registry.find_service(service_name).await.is_none() {
            return Err(StoreError::ServiceNotFound(service_name.to_string()));
        }
    ...
    pub async fn show_config(&self) -> Result<serde_json::Value> {
        if self.source_mode == SourceMode::Db {
            ...
            let mut config = mcpstore_config::McpConfig::default();
    ...
    pub async fn reset_config(&self) -> Result<()> {
        ...
        if self.source_mode == SourceMode::Local {
            self.config_manager
                .save(&mcpstore_config::McpConfig::default())?;
        }
        Ok(())
    }
    ...
    pub async fn check_services_scoped(&self, agent_id: Option<&str>) -> Result<serde_json::Value> {
        let services = self.list_services_scoped(agent_id).await?;
        ...
        for service in services {
            let service_key = service
                ...
                .ok_or_else(|| {
                    StoreError::Other("Scoped service payload missing name".to_string())
                })?;
            let global_service_name = service
                ...
                .ok_or_else(|| {
                    StoreError::Other(
                        "Scoped service payload missing global identifier".to_string(),
                    )
                })?;
            let status = self.health_check(global_service_name).await?;
            result.insert(
                service_key.to_string(),
                serde_json::json!(status.health_status),
            );
        }
    ...
    async fn cache_service_connected(
        &self,
        name: &str,
        tools: &[mcpstore_registry::ToolInfo],
    ) -> Result<()> {
        if self.source_mode == SourceMode::Db {
            return Ok(());
        }
    ...
    async fn put_service_status_payload(&self, payload: &ServiceStatus) -> Result<()> {
        if self.source_mode == SourceMode::Db {
            return Ok(());
        }
    ...
    fn retry_wait_seconds(status: &ServiceStatus, now: f64) -> Option<i64> {
    ...
    fn retry_exhausted(status: &ServiceStatus, now: f64) -> bool {
    ...
    fn retry_poll_interval(status: &ServiceStatus) -> std::time::Duration {
        let now = Self::now_timestamp_f64();
    ...
    async fn hydrate_service_status_from_cache(&self, service: &mut ServiceEntry) -> Result<()> {
    ...
    async fn mark_service_retryable_failure(
        &self,
        name: &str,
        error: String,
    ) -> Result<ServiceStatus> {
    ...
    async fn sync_retry_window(&self, name: &str) -> Result<Option<ServiceStatus>> {
