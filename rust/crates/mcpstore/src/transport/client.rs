...
fn content_item_from_rmcp(content: rmcp::model::Content) -> ContentItem {
...
mod tests {
    use super::*;
    use rmcp::model::{
        AnnotateAble, RawAudioContent, RawContent, RawResource, ResourceContents,
    };

    #[test]
    fn converts_non_text_tool_content_without_downgrading() {
        let image = content_item_from_rmcp(
            RawContent::image("image-data", "image/png").no_annotation(),
        );
        match image {
            ContentItem::Image {
                data, mime_type, ..
        ...
        let resource = content_item_from_rmcp(
            RawContent::resource(ResourceContents::text("body", "memory://doc"))
                .no_annotation(),
        );
        match resource {
