...
use mcpstore_core::{perspective::GLOBAL_AGENT_STORE, store::MCPStore};
use rmcp::{
    model::{
    ...
    fn call_tool(
        ...
        async move {
            ...
            for item in result.content {
                match item {
                    mcpstore_transport::ContentItem::Text { text } => {
                        content.push(Content::text(text));
                    }
                    mcpstore_transport::ContentItem::Image { data, mime_type } => {
...
fn map_store_error(error: mcpstore_core::StoreError) -> ErrorData {
