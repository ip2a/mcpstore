...
use mcpstore_core::store::MCPStore;
    ...
    match store.call_tool(&service_name, &tool_name, args).await {
        Ok(result) => DaemonResponse::ok(Some(json!({
            "content": result.content.iter().map(|c| match c {
                mcpstore_transport::ContentItem::Text { text } => json!({"type": "text", "text": text}),
                mcpstore_transport::ContentItem::Image { data, mime_type } => json!({"type": "image", "data": data, "mime_type": mime_type}),
            }).collect::<Vec<_>>(),
