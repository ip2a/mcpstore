...
提供与 MCPStore 官方 `CallToolResult` 完全兼容的失败结果封装，保证调用链
无论成功还是失败都能拿到统一的数据结构。
"""
...
class CallToolFailureResult:
    """
    MCPStore CallToolResult 的失败封装。

    通过标准的文本内容块返回错误信息，同时补齐 MCPStore 客户端常用的
    ...
    def __post_init__(self) -> None:
        ...
        # MCPStore 官方对象同时会暴露蛇形和驼峰字段，这里补齐常用别名
    ...
    def unwrap(self) -> mcp_types.CallToolResult:
        """返回 MCPStore 官方 CallToolResult 对象。"""
        return self._result
