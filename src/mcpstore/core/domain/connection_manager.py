...
class ConnectionManager:
    ...
    def __init__(
        ...
        logger.info("ConnectionManager initialized and subscribed to events")
    ...
    async def _on_service_initialized(self, event: ServiceInitialized):
        ...
        await self._event_bus.publish(connection_request)
    
    async def _on_connection_requested(self, event: ServiceConnectionRequested):
        """
        处理连接请求 - 执行实际连接
        """
        logger.info(f"[CONNECTION] Connecting to: {event.service_name}")
