...
def load_and_merge_config(
    ...
    **cli_overrides,
    ...
    # Auto-detect mcpstore.mcp.json if no server_spec provided
    if server_spec is None:
        config_path = Path("mcpstore.mcp.json")
        if not config_path.exists():
            ...
            if found_config:
                ...
                logger.error(
                    "No server specification provided and no mcpstore.mcp.json found in current directory.\n"
                    "Please specify a server file or create a mcpstore.mcp.json configuration."
                )
                raise FileNotFoundError("No server specification or mcpstore.mcp.json found")
