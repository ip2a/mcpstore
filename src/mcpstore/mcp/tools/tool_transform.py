...
from mcpstore.mcp.utilities.json_schema import compress_schema
...
class ArgTransformConfig(MCPStoreBaseModel):
    ...
    def to_arg_transform(self) -> ArgTransform:
        """Convert the argument transform to a MCPStore argument transform."""
...
class TransformedTool(Tool):
        ...
        try:
            ...
            if isinstance(result, ToolResult):
                if self.output_schema is None:
                    ...
                    "type"
                ) != "object" and not self.output_schema.get("x-mcpstore-wrap-result"):
            ...
            if self.output_schema is not None:
                if self.output_schema.get("x-mcpstore-wrap-result"):
    ...
    def from_tool(
        ...
        if (
            ...
            and settings.deprecation_warnings
        ):
            warnings.warn(
...
def _set_visibility_metadata(tool: Tool, *, enabled: bool) -> None:
    ...
    from mcpstore.mcp.server.transforms.visibility import _MCPSTORE_KEY, _INTERNAL_KEY

    if tool.meta is None:
        tool.meta = {_MCPSTORE_KEY: {_INTERNAL_KEY: {"visibility": enabled}}}
    else:
        old_mcpstore = tool.meta.get(_MCPSTORE_KEY, {})
        old_internal = old_mcpstore.mcp.get(_INTERNAL_KEY, {})
        new_internal = {**old_internal, "visibility": enabled}
        new_mcpstore = {**old_mcpstore, _INTERNAL_KEY: new_internal}
        tool.meta = {**tool.meta, _MCPSTORE_KEY: new_mcpstore}


class ToolTransformConfig(MCPStoreBaseModel):
