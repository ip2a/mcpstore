...
from mcpstore.mcp.server.auth import AuthProvider
from mcpstore.mcp.server.auth.middleware import RequireAuthMiddleware
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        try:
            ...
            if str(e) == "Task group is not initialized. Make sure to use run().":
                ...
                new_error_message = (
                    "MCPStore's StreamableHTTPSessionManager task group was not initialized. "
                    "This commonly occurs when the MCPStore application's lifespan is not "
                    ...
                    "returned by `mcpstore_instance.http_app()`. \\n"
                    "For more details, see the MCPStore ASGI integration documentation: "
                    "https://gomcpstore.mcp.com/deployment/asgi"
                )
...
def create_sse_app(
    ...
    # Store the MCPStore server instance on the Starlette app state
    app.state.mcpstore_server = server
