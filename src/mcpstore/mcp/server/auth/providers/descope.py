"""Descope authentication provider for MCPStore.
...
class DescopeProvider(RemoteAuthProvider):
    ...
    as it allows Descope to handle the OAuth flow directly while MCPStore acts
    ...
    Example:
        ```python
        from mcpstore.mcp.server.auth.providers.descope import DescopeProvider
        ...
        # Use with MCPStore
        mcp = MCPStore("My App", auth=descope_auth)
        ```
    """
    ...
    def get_routes(
        ...
        async def oauth_authorization_server_metadata(request):
            """Forward Descope OAuth authorization server metadata with MCPStore customizations."""
            try:
