"""Lifespan and Docket task infrastructure for MCPStore Server."""
...
class LifespanMixin:
    """Mixin providing lifespan and Docket task infrastructure for MCPStore."""

    @property
    def docket(self: MCPStore) -> Docket | None:
    ...
    async def _docket_lifespan(self: MCPStore) -> AsyncIterator[None]:
        ...
        """
        from mcpstore.mcp.server.dependencies import _current_server, is_docket_available

        # Set MCPStore server in ContextVar so CurrentMCPStore can access it
        ...
        try:
            ...
            async with Docket(
                ...
                # Store on server instance for cross-task access (MCPStoreTransport)
                self._docket = docket
    ...
    @asynccontextmanager
    async def _lifespan_manager(self: MCPStore) -> AsyncIterator[None]:
    ...
    def _setup_task_protocol_handlers(self: MCPStore) -> None:
        ...
        """
        from mcpstore.mcp.server.dependencies import is_docket_available
        ...
        from mcpstore.mcp.server.tasks.requests import (
