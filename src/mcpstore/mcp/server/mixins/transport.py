"""Transport-related methods for MCPStore Server."""
...
from mcpstore.mcp.utilities.cli import log_server_banner
from mcpstore.mcp.utilities.logging import get_logger, temporary_log_level

if TYPE_CHECKING:
    from mcpstore.mcp.server.server import MCPStore, Transport

logger = get_logger(__name__)


class TransportMixin:
    """Mixin providing transport-related methods for MCPStore.
    ...
    async def run_async(
        ...
        **transport_kwargs: Any,
        ...
        """Run the MCPStore server asynchronously.
        ...
        Args:
            ...
            show_banner: Whether to display the server banner. If None, uses the
                MCPSTORE_SHOW_SERVER_BANNER setting (default: True).
        """
    ...
    def run(
        ...
        **transport_kwargs: Any,
        ...
        """Run the MCPStore server. Note this is a synchronous function.
    ...
    def custom_route(
        ...
        """
        Decorator to register a custom HTTP route on the MCPStore server.
    ...
    def _get_additional_http_routes(self: MCPStore) -> list[BaseRoute]:
        """Get all additional HTTP routes including from providers.

        Returns a list of all custom HTTP routes from this server and
        from all providers that have HTTP routes (e.g., MCPStoreProvider).
    ...
    async def run_stdio_async(
        ...
        """
        from mcpstore.mcp.server.context import reset_transport, set_transport
