...
from mcpstore.mcp.prompts.prompt import PromptResult
...
from mcpstore.mcp.server.apps import (
    ResourceUI,
    ToolUI,
    resolve_ui_mime_type,
    ui_to_meta_dict,
)
from mcpstore.mcp.server.auth import AuthContext, AuthProvider, run_auth_checks
from mcpstore.mcp.server.dependencies import get_access_token
from mcpstore.mcp.server.lifespan import Lifespan
from mcpstore.mcp.server.low_level import LowLevelServer
from mcpstore.mcp.server.middleware import Middleware, MiddlewareContext
from mcpstore.mcp.server.mixins import LifespanMixin, MCPOperationsMixin, TransportMixin
from mcpstore.mcp.server.providers import LocalProvider, Provider
from mcpstore.mcp.server.providers.aggregate import AggregateProvider
from mcpstore.mcp.server.tasks.config import TaskConfig, TaskMeta
from mcpstore.mcp.server.telemetry import server_span
from mcpstore.mcp.server.transforms import (
    ToolTransform,
    Transform,
)
from mcpstore.mcp.server.transforms.visibility import apply_session_transforms, is_enabled
from mcpstore.mcp.settings import DuplicateBehavior as DuplicateBehaviorSetting
from mcpstore.mcp.settings import Settings
from mcpstore.mcp.tools.function_tool import FunctionTool
from mcpstore.mcp.tools.tool import AuthCheckCallable, Tool, ToolResult
from mcpstore.mcp.tools.tool_transform import ToolTransformConfig
from mcpstore.mcp.utilities.components import MCPStoreComponent
from mcpstore.mcp.utilities.logging import get_logger
from mcpstore.mcp.utilities.types import MCPStoreBaseModel, NotSet, NotSetT
from mcpstore.mcp.utilities.versions import (
    VersionSpec,
)
...
if TYPE_CHECKING:
    ...
    from mcpstore.mcp.server.providers.openapi import RouteMap
    from mcpstore.mcp.server.providers.openapi import RouteMapFn as OpenAPIRouteMapFn
    from mcpstore.mcp.server.providers.proxy import MCPStoreProxy
    from mcpstore.mcp.tools.tool import ToolResultSerializerType
...
def _get_auth_context() -> tuple[bool, Any]:
    ...
    """
    from mcpstore.mcp.server.context import _current_transport
...
def _lifespan_proxy(
    ...
    async def wrap(
        ...
        if mcpstore_server._lifespan is default_lifespan:
            yield {}
            return
        ...
        yield mcpstore_server._lifespan_result

    return wrap


class StateValue(MCPStoreBaseModel):
...
class MCPStore(
    ...
    def __init__(
        ...
        *,
        ...
        self._state_store: PydanticAdapter[StateValue] = PydanticAdapter[StateValue](
            ...
            default_collection="mcpstore_state",
        )
        ...
        if tool_serializer is not None and settings.deprecation_warnings:
            warnings.warn(
                "The `tool_serializer` parameter is deprecated. "
                "Return ToolResult from your tools for full control over serialization. "
                "See https://gomcpstore.mcp.com/servers/tools#custom-serialization for migration examples.",
                DeprecationWarning,
                stacklevel=2,
            )
        ...
        self._mcp_server: LowLevelServer[LifespanResultT, Any] = LowLevelServer[
            ...
            version=version or mcpstore_mcp.__version__,
            instructions=instructions,
            website_url=website_url,
            icons=icons,
            lifespan=_lifespan_proxy(mcpstore_server=self),
        )
        ...
        self.strict_input_validation: bool = (
            ...
            else settings.strict_input_validation
        )
    ...
    def _handle_deprecated_settings(
        ...
        combined_settings = settings.model_dump() | deprecated_settings
    ...
    def settings(self) -> Settings:
        ...
        if settings.deprecation_warnings:
            warnings.warn(
                "Accessing `.settings` on a MCPStore instance is deprecated. Use the global `settings` instead.",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    async def get_tasks(self) -> Sequence[MCPStoreComponent]:
    ...
    async def list_tools(self, *, run_middleware: bool = True) -> Sequence[Tool]:
        ...
        """
        async with mcpstore_mcp.server.context.Context(mcpstore=self) as ctx:
            if run_middleware:
    ...
    def mount(
        self,
        server: MCPStore[LifespanResultT],
        namespace: str | None = None,
        as_proxy: bool | None = None,
        tool_names: dict[str, str] | None = None,
        prefix: str | None = None,  # deprecated, use namespace
    ) -> None:
        """Mount another MCPStore server on this server with an optional namespace.
        ...
        Args:
            server: The MCPStore server to mount.
        ...
        if as_proxy is not None:
            ...
            if as_proxy:
                ...
                if not isinstance(server, MCPStoreProxy):
                    server = MCPStore.as_proxy(server)

        # Create provider and add it with namespace
        provider: Provider = MCPStoreProvider(server)
    ...
    async def import_server(
        ...
        """
        Import the MCP objects from another MCPStore server into this one,
        ...
        Args:
            server: The MCPStore server to import
    ...
    def from_openapi(
        ...
        **settings: Any,
        ...
        """
        Create a MCPStore server from an OpenAPI specification.
        ...
        Args:
            ...
            **settings: Additional settings passed to MCPStore

        Returns:
            A MCPStore server with an OpenAPIProvider attached.
        """
    ...
    def from_fastapi(
        ...
        **settings: Any,
        ...
        """
        Create a MCPStore server from a FastAPI application.
    ...
    def as_proxy(
        ...
        **settings: Any,
        ...
        The `backend` argument can be either an existing `mcpstore.mcp.client.Client`
        instance or any value accepted as the `transport` argument of
        `mcpstore.mcp.client.Client`. This mirrors the convenience of the
        `mcpstore.mcp.client.Client` constructor.
        """
        if settings.deprecation_warnings:
            warnings.warn(
...
def create_proxy(
    ...
    **settings: Any,
    ...
    Args:
        ...
        **settings: Additional settings passed to MCPStoreProxy (name, etc.)

    Returns:
        A MCPStoreProxy server that proxies to the target.

    Example:
        ```python
        from mcpstore.mcp.server import create_proxy
        ...
        ```
    """
    from mcpstore.mcp.server.providers.proxy import (
        MCPStoreProxy,
        _create_client_factory,
    )

    client_factory = _create_client_factory(target)
    return MCPStoreProxy(
