...
from mcpstore.mcp.client.telemetry import client_span
from mcpstore.mcp.client.transports import ClientTransportT
from mcpstore.mcp.exceptions import ResourceError, ToolError
from mcpstore.mcp.mcp_config import MCPConfig
from mcpstore.mcp.prompts import Message, Prompt, PromptResult
from mcpstore.mcp.prompts.prompt import PromptArgument
from mcpstore.mcp.resources import Resource, ResourceTemplate
from mcpstore.mcp.resources.resource import ResourceContent, ResourceResult
...
if TYPE_CHECKING:
    ...
    from mcpstore.mcp.client.transports import ClientTransport
...
class ProxyTool(Tool):
    ...
    def from_mcp_tool(
        ...
        return cls(
            ...
            meta=mcp_tool.meta,
            tags=get_mcpstore_metadata(mcp_tool.meta).get("tags", []),
        )
    ...
    async def run(
        ...
        with client_span(
            ...
            span.set_attribute("mcpstore.mcp.provider.type", "ProxyProvider")
    ...
    def get_span_attributes(self) -> dict[str, Any]:
        return super().get_span_attributes() | {
            "mcpstore.mcp.provider.type": "ProxyProvider",
            "mcpstore.mcp.proxy.backend_name": self._backend_name,
        }
...
class ProxyResource(Resource):
    ...
    def from_mcp_resource(
        ...
        return cls(
            ...
            tags=get_mcpstore_metadata(mcp_resource.meta).get("tags", []),
    ...
    def get_span_attributes(self) -> dict[str, Any]:
        return super().get_span_attributes() | {
            "mcpstore.mcp.provider.type": "ProxyProvider",
            "mcpstore.mcp.proxy.backend_uri": self._backend_uri,
        }
...
class ProxyTemplate(ResourceTemplate):
    ...
    def from_mcp_template(  # type: ignore[override]
        ...
        return cls(
            ...
            tags=get_mcpstore_metadata(mcp_template.meta).get("tags", []),
            task_config=TaskConfig(mode="forbidden"),
        )
    ...
    async def create_resource(
        ...
        return ProxyResource(
            ...
            icons=self.icons,
            meta=self.meta,
            tags=get_mcpstore_metadata(self.meta).get("tags", []),
    ...
    def get_span_attributes(self) -> dict[str, Any]:
        return super().get_span_attributes() | {
            "mcpstore.mcp.provider.type": "ProxyProvider",
            "mcpstore.mcp.proxy.backend_uri_template": self._backend_uri_template,
        }
...
class ProxyPrompt(Prompt):
    ...
    def from_mcp_prompt(
        ...
        return cls(
            ...
            tags=get_mcpstore_metadata(mcp_prompt.meta).get("tags", []),
            task_config=TaskConfig(mode="forbidden"),
        )
    ...
    async def render(self, arguments: dict[str, Any]) -> PromptResult:  # type: ignore[override]
        ...
        with client_span(
            ...
            # (not the static prompt meta which includes mcpstore tags)
...
# -----------------------------------------------------------------------------
# MCPStoreProxy - Convenience Wrapper
# -----------------------------------------------------------------------------
...
class MCPStoreProxy(MCPStore):
    ...
    Example:
        ...
        from mcpstore.mcp.server.providers.proxy import MCPStoreProxy, ProxyClient

        # Create a proxy server using create_proxy (recommended)
        proxy = create_proxy("http://localhost:8000/mcp")

        # Or use MCPStoreProxy directly with explicit client factory
        proxy = MCPStoreProxy(client_factory=lambda: ProxyClient("http://localhost:8000/mcp"))
        ```
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        MCPStoreProxy requires explicit session management via client_factory.
