...
This directory contains the next-generation MCPStore server implementation for OpenAPI integration, designed to replace the legacy implementation in `/server/openapi.py`.
...
1. **`server.py`** - `MCPStoreOpenAPI` main server class with RequestDirector integration
...
```python
from mcpstore.mcp.server.openapi_new.routing import RouteMap
...
async def test_stateless_request_building():
    ...
    server = MCPStoreOpenAPI(spec=valid_spec, client=client)
...
import logging
logging.getLogger("mcpstore.mcp.server.openapi_new").setLevel(logging.DEBUG)
```
