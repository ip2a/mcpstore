"""
HTTP routes for enabling/disabling components in MCPStore.
...
def set_up_component_manager(
    server: MCPStore, path: str = "/", required_scopes: list[str] | None = None
) -> None:
...
def _build_routes(server: MCPStore, base_path: str) -> list[Route]:
...
def _make_endpoint(server: MCPStore, component_type: str, action: str):
