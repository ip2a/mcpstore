...
def create_client_server(url: str) -> Any:
    """Create a MCPStore server from a client URL.
    ...
    try:
        ...
        client = mcpstore.mcp.Client(url)
...
def create_mcp_config_server(mcp_config_path: Path) -> MCPStore[None]:
    """Create a MCPStore server from a MCPConfig."""
...
def load_mcp_server_config(config_path: Path) -> MCPServerConfig:
    """Load a MCPStore configuration from a mcpstore.mcp.json file.

    Args:
        config_path: Path to mcpstore.mcp.json file
...
async def run_command(
    ...
    # Special case: MCPConfig files (legacy)
        ...
        if "mcpServers" in data:
            ...
            # It's a MCPStore config - load it properly
            ...
            from mcpstore.mcp.cli.cli import with_argv
