"""MCPStore CLI tools using Cyclopts."""
...
from mcpstore.mcp.utilities.cli import is_already_in_uv_subprocess, load_and_merge_config
from mcpstore.mcp.utilities.inspect import (
    InspectFormat,
    format_info,
    inspect_mcpstore,
)
...
app = cyclopts.App(
    name="mcpstore",
    help="MCPStore 2.0 - The fast, Pythonic way to build MCP servers and clients.",
    version=mcpstore.mcp.__version__,
...
def version(
    *,
    ...
    info = {
        "MCPStore version": mcpstore.mcp.__version__,
        "MCP version": importlib.metadata.version("mcp"),
        "Python version": platform.python_version(),
        "Platform": platform.platform(),
        "MCPStore root path": Path(mcpstore.mcp.__file__ or ".").resolve().parents[1],
    }
...
async def dev(
    ...
    *,
    ...
    Args:
        server_spec: Python file to run, optionally with :object suffix, or None to auto-detect mcpstore.mcp.json
    """
    ...
    try:
        ...
        # Build the mcpstore run command
        mcpstore_cmd = ["mcpstore", "run", server_spec, "--no-banner"]

        # Add reload flags if enabled - the server will handle reloading
        if reload:
            mcpstore_cmd.append("--reload")
            if reload_dir:
                for dir_path in reload_dir:
                    mcpstore_cmd.extend(["--reload-dir", str(dir_path)])

        # Use the environment from config (already has CLI overrides applied)
        uv_cmd = config.environment.build_command(mcpstore_cmd)

        # Set marker to prevent infinite loops when subprocess calls MCPStore
        env = dict(os.environ.items()) | env_vars | {"MCPSTORE_UV_SPAWNED": "1"}
...
async def run(
    ...
    *server_args: str,
    ...
    5. MCPStore config: "mcpstore.mcp.json" - runs server using MCPStore configuration
    6. No argument: looks for mcpstore.mcp.json in current directory

    Server arguments can be passed after -- :
    mcpstore run server.py -- --config config.json --debug
    ...
    final_no_banner = (
        no_banner if no_banner else not settings.show_server_banner
    )
    ...
    if reload:
        ...
        if final_transport == "sse":
            ...
            reload_cmd = ["mcpstore", "run", server_spec]
            if final_transport:
    ...
    if needs_uv:
        # Build the inner mcpstore command
        inner_cmd = ["mcpstore", "run", server_spec]
        ...
        # Set marker to prevent infinite loops when subprocess calls MCPStore again
        env = os.environ | {"MCPSTORE_UV_SPAWNED": "1"}
...
async def inspect(
    ...
    *,
    format: Annotated[
        ...
        cyclopts.Parameter(
            name=["--format", "-f"],
            help="Output format: mcpstore (MCPStore-specific) or mcp (MCP protocol). Required when using -o.",
        ),
    ] = None,
    output: Annotated[
        Path | None,
    ...
    Examples:
        # Show text summary
        mcpstore inspect server.py

        # Output MCPStore format JSON to stdout
        mcpstore inspect server.py --format mcpstore

        # Save MCP protocol format to file (format required with -o)
        mcpstore inspect server.py --format mcp -o manifest.json

        # Inspect from mcpstore.mcp.json configuration
        mcpstore inspect mcpstore.mcp.json
        mcpstore inspect  # auto-detect mcpstore.mcp.json

    Args:
        server_spec: Python file to inspect, optionally with :object suffix, or mcpstore.mcp.json
    """
    ...
    try:
        ...
        info = await inspect_mcpstore(server)
        ...
        if output and not format:
            ...
            console.print(
                "[dim]Use --format mcpstore or --format mcp to specify the output format[/dim]"
            )
            sys.exit(1)
        ...
        if format is None:
            ...
            console.print(f"  MCPStore:      {info.mcpstore_version}")
            console.print(f"  MCP:          {info.mcp_version}")
            console.print()

            console.print(
                "[dim]Use --format \\[mcpstore|mcp] for complete JSON output[/dim]"
            )
            return
...
project_app = cyclopts.App(name="project", help="Manage MCPStore projects")
...
async def prepare(
    config_path: Annotated[
        str | None,
        cyclopts.Parameter(help="Path to mcpstore.mcp.json configuration file"),
    ] = None,
    ...
    """Prepare a MCPStore project by creating a persistent uv environment.
    ...
    mcpstore run <config> --project <output-dir>
    ...
    Example:
        mcpstore project prepare myserver.json --output-dir ./prepared-env
        mcpstore run myserver.json --project ./prepared-env
    """
    from pathlib import Path
    ...
    # Auto-detect mcpstore.mcp.json if not provided
    if config_path is None:
        ...
        if found_config:
            ...
            logger.error(
                "No configuration file specified and no mcpstore.mcp.json found.\n"
                "Please specify a configuration file or create a mcpstore.mcp.json."
            )
            sys.exit(1)
    ...
    try:
        ...
        console.print(
            ...
            f"  [cyan]mcpstore run {config_path} --project {output_dir}[/cyan]"
        )
