"""MCPStore tasks CLI for Docket task management."""
...
from mcpstore.mcp.utilities.cli import load_and_merge_config
...
tasks_app = cyclopts.App(
    name="tasks",
    help="Manage MCPStore background tasks using Docket",
)
...
def check_distributed_backend() -> None:
    ...
    if docket_url.startswith("memory://"):
        console.print(
            ...
            "To use [cyan]mcpstore tasks[/cyan] CLI commands (which run in separate\n"
            ...
            "   export MCPSTORE_DOCKET_URL=redis://localhost:6379/0\n\n"
            ...
            "Need help? See: [cyan]https://gomcpstore.mcp.com/docs/tasks[/cyan]"
        )
        sys.exit(1)
...
def worker(
    server_spec: Annotated[
        str | None,
        cyclopts.Parameter(
            help="Python file to run, optionally with :object suffix, or None to auto-detect mcpstore.mcp.json"
        ),
    ] = None,
) -> None:
    ...
    async def run_worker():
        ...
        async with server._lifespan_manager():
            ...
            console.print(f"  Docket: {settings.docket.name}")
            console.print(f"  Backend: {settings.docket.url}")
            console.print(f"  Concurrency: {settings.docket.concurrency}")
