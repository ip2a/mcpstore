...
if TYPE_CHECKING:
    from mcpstore.mcp.client.transports import (
        ClientTransport,
        SSETransport,
        StdioTransport,
...
class _TransformingMCPServerMixin(MCPStoreBaseModel):
    ...
    def _to_server_and_underlying_transport(
    ...
    ) -> tuple[MCPStore[Any], ClientTransport]:
        """Turn the Transforming MCPServer into a MCPStore Server and also return the underlying transport."""
        ...
        if self.tools:
            from mcpstore.mcp.server.transforms import ToolTransform
    ...
    def to_transport(self) -> ClientTransport:
        ...
        return MCPStoreTransport(mcp=self._to_server_and_underlying_transport()[0])
...
class StdioMCPServer(BaseModel):
    ...
    def to_transport(self) -> StdioTransport:
        from mcpstore.mcp.client.transports import StdioTransport
...
class RemoteMCPServer(BaseModel):
    ...
    def to_transport(self) -> StreamableHttpTransport | SSETransport:
        from mcpstore.mcp.client.transports import SSETransport, StreamableHttpTransport
...
class MCPConfig(BaseModel):
    ...
    while adding additional fields for enabling MCPStore-specific features like tool transformations
