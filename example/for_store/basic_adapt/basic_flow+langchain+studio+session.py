...
# LangChain + Studio + Session Mode
# ============================================================

print("\n" + "=" * 60)
print("  MCPStore Standard Workflow Example (LangChain + Studio + Session Mode)")
print("=" * 60)
...
# ------------------------------------------------------------
# Step 6: Create Session and Bind Service
# ------------------------------------------------------------
print("\n[Step 6] Create Session and Bind Service")
print("  ├─ Session Mode Features:")
print("  │   ├─ Automatic resource management")
print("  │   ├─ State persistence")
print("  │   └─ Support for chained tool calls")

# Use with statement to create session context (automatic cleanup)
with store.for_store().with_session(session_name) as session:
    print(f"  └─ ✓ Session created: {session.session_id}")

    # Bind service to session
    print("\n[Step 6.5] Bind Service to Session")
    session.bind_service(service_name)
    print(f"  ├─ Service Name: {service_name}")
    print(f"  ├─ Session ID: {session.session_id}")
    print("  └─ ✓ Service bound to session successfully")

    # ------------------------------------------------------------
    # Step 7: List All Tools (in Session Context)
    # ------------------------------------------------------------
    print("\n[Step 7] List All Tools (in Session Context)")
    ...
    # ------------------------------------------------------------
    # Step 8: Setup LangChain Agent and Call Tools (in Session Context)
    # ------------------------------------------------------------
    print("\n[Step 8] Setup LangChain Agent and Call Tools (in Session Context)")

    # 8.1: Load LangChain tools
    ...
    print("\n  ✓ Session operations completed, exiting context (automatic cleanup)")

# Session context exited, resources automatically cleaned up

# ------------------------------------------------------------
# Step 9: Reset Configuration (Final Cleanup - Show Before/After)
# ------------------------------------------------------------
print("\n[Step 9] Reset Configuration (Final Cleanup - Show Before/After)")
...
# ============================================================
print("\n" + "=" * 60)
print("  Standard Workflow Completed (LangChain + Studio + Session Mode)")
print("=" * 60)
print()
