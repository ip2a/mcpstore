...
// Stores for refreshing data without full reload
...
const switchWorkspace = async (name) => {
  try {
    ...
    if (response.data?.success) {
      ...
      // Refresh other app data without full page reload
      try {
        await Promise.all([
          systemStore.safeCheckSystemStatus?.() || systemStore.checkServices?.() || Promise.resolve(),
          servicesStore.refreshAll?.() || servicesStore.fetchServices?.(true) || Promise.resolve(),
          toolsStore.fetchTools?.(true) || Promise.resolve()
        ])
      } catch (e) {
        console.warn('刷新关联数据时出现问题（已忽略整页刷新）:', e)
      }
    }
  } catch (error) {
    if (error !== 'cancel') {
