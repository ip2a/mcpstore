...
export function OverrideEditor({ kind, serviceName, componentKey, scope, existingOverride, onSuccess }: Props) {
  ...
  const tools = useToolOverridesQuery(); const prompts = usePromptOverridesQuery(); const resources = useResourceOverridesQuery(); const templates = useResourceTemplateOverridesQuery()
  ...
  const [open, setOpen] = useState(false); const [displayName, setDisplayName] = useState(""); const [description, setDescription] = useState(""); const [enabled, setEnabled] = useState(true); const [mimeType, setMimeType] = useState("")
  ...
  function patch() {
    const body: Record<string, unknown> = {}; if (displayName !== (rule?.display_name || "")) body.display_name = displayName; if (description !== (rule?.description || "")) body.description = description; if (enabled !== (rule?.enabled !== false)) body.enabled = enabled; if (kind === "resource" || kind === "resource_template") { const old = (rule as ResourceOverrideRule | ResourceTemplateOverrideRule | undefined)?.mime_type || ""; if (mimeType !== old) body.mime_type = mimeType }
    return body
  }
  async function save() { const body = patch(); if (kind === "tool") await setTool.mutateAsync(body); else if (kind === "prompt") await setPrompt.mutateAsync(body); else if (kind === "resource") await setResource.mutateAsync(body); else await setTemplate.mutateAsync(body); await client.invalidateQueries({ queryKey: queryKeys.overrides(kind) }); toast.success(t("saved")); onSuccess?.() }
  async function remove() { if (kind === "tool") await delTool.mutateAsync(); else if (kind === "prompt") await delPrompt.mutateAsync(); else if (kind === "resource") await delResource.mutateAsync(); else await delTemplate.mutateAsync(); await client.invalidateQueries({ queryKey: queryKeys.overrides(kind) }); setOpen(false); toast.success(t("delete")); onSuccess?.() }
